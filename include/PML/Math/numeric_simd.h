...
namespace pml {
    ...
    */
    template<typename T, typename Allocator>
    T inner_product_SIMD(
        const std::vector<T, Allocator>& inA,
        const std::vector<T, Allocator>& inB,
        T val)
    {
        if (CPUDispatcher::isAVX())
        {
            if constexpr (std::is_same<Allocator, pml::aligned::detail::aligned_allocator<T>>::value)
            {
                return val + aligned::detail::inner_product_AVX_Impl(
        ...
        return std::inner_product(inA.cbegin(), inA.cend(), inB.cbegin(), val);
    }
